const AppData = {
    ...
    saveBookmark(bookmark) {
        ...
        const exists = bookmarks.some(b => b.title === bookmark.title);
        if (!exists) {
            bookmarks.unshift(bookmark);
            localStorage.setItem('zr_bookmarks', JSON.stringify(bookmarks));
        }
        return !exists;
    },
...
const MockData = {
    ...
    getMockMonthRecords() {
        ...
        const mockDays = [1,3,4,6,8,9,11,13,14,16,18,19,20,22,24,25];
        const symptomSets = [
            {1:3,2:2,3:1,4:2},
            {1:2,3:3,5:1,7:1},
            {2:2,4:3,6:1},
        ...
        const moods = ['平静','有点烦','疲惫','难过','平静','有点烦','开心','疲惫','有点烦','难过','疲惫','平静','开心','难过','有点烦','平静'];
        mockDays.forEach((day, idx) => {
            const d = new Date(year, month, day);
            const key = d.toISOString().slice(0, 10);
            records[key] = { symptoms: symptomSets[idx % symptomSets.length], mood: moods[idx % moods.length], date: key };
        });
        return records;
    },

    generateWeekData() {
        let records = AppData.getAllRecords();
        if (Object.keys(records).length === 0) {
            records = this.getMockMonthRecords();
        }
        const week = [];
        for (let i = 6; i >= 0; i--) {
            const d = new Date();
    ...
    shareTemplates: {
        partner: {
            '温和说明': (symptoms) => `最近我有时会${symptoms || '感到不舒服'}，这并不是我故意情绪不好，也不是想把压力转给你。\n\n这段时间身体的变化可能会影响睡眠、精力和情绪稳定。我正在尝试记录和调整。\n\n如果可以，我希望你在我状态不太好时，先不要急着评价或讲道理。你可以问我："你现在是想休息一下，还是想让我陪你说说话？"`,
            '认真沟通': (symptoms) => `我想跟你认真地聊一聊我最近的身体状态。\n\n根据这段时间的记录，我在${symptoms || '身体感受'}方面有一些明显变化。这不是主观的"心情不好"，而是身体正在经历一个激素调整的阶段。\n\n我需要你理解的是：这些变化不是我能完全控制的。我需要的是倾听和支持，而不是评价和建议。如果你愿意，我们可以一起了解更多。`,
            '简短提醒': () => `最近身体有些变化，可能会比较容易累、容易烦躁。不是你的问题，也不是针对你。\n\n你能做的最好的支持：多一点点耐心，少一点点评价。谢谢你。`,
            '轻松一点': () => `嘿，最近身体在跟我"闹小脾气"呢 :)\n\n有时候会突然出汗、睡不好，情绪也会莫名其妙地低落。不是我想这样的啦！\n\n跟你说一声，是希望你知道：如果哪天我状态不太好，给我一个拥抱或者一杯热茶就好~`
        },
        child: {
            '温和说明': () => `妈妈最近可能会更容易疲惫、睡不好，情绪也会有些波动。\n\n这不是她故意发脾气，也不是"想太多"。她的身体正在经历一个需要适应的阶段。\n\n你能做的支持很简单：少一点评价，多一点耐心；如果她愿意说，就认真听她说完。`,
            '认真沟通': (symptoms) => `我想让你了解一些关于我身体的事情。\n\n最近我经历了${symptoms || '一些身体变化'}——这些都是围绝经期常见的现象，是身体激素水平变化带来的。\n\n我需要你知道：这不是"老了"或"脾气不好"，而是身体在经历一个自然的转折。你不需要"解决"什么，只需要多一些理解和耐心。`,
            '简短提醒': () => `妈妈最近身体有些变化，可能会比以前更容易累、更需要休息。\n\n这不是你做得不够好，只是身体需要一些时间来适应。如果可以的话，多跟我说说话，或者只是安静地陪我坐一会儿。`,
            '轻松一点': () => `宝贝，妈妈想跟你说：最近我的身体像在"系统升级"，有时候会卡顿、发热、还会蓝屏 :D\n\n所以有时候可能会表现得有点奇怪~ 不是针对你哦！如果妈妈看起来累了，帮我倒杯水就是最大的支持~`
        },
        friend: {
            '温和说明': (symptoms) => `想跟你分享一件事。最近我的身体经历了一些变化，主要是${symptoms || '睡眠和情绪方面'}。\n\n这不是什么严重的问题，但我希望你知道——有时候我可能会突然取消计划或者回复很慢，不是因为不在乎，而是身体需要休息。\n\n谢谢你的理解和陪伴。`,
            '认真沟通': (symptoms) => `作为朋友，我想让你了解我最近的状态。\n\n${symptoms || '身体不适'}是我最近经历的主要变化。这可能和围绝经期的激素波动有关。\n\n我需要朋友的理解——不是同情，而是尊重我可能需要更多休息、更需要被倾听的这段时间。`,
            '简短提醒': () => `最近身体在经历一些变化，可能会比以前更容易累。如果哪天我临时放鸽子，请原谅我~不是不想见你，是身体需要休息。`,
            '轻松一点': () => `姐妹们，跟你们说个事！我的身体最近像个"变频空调"——忽冷忽热、随机切换模式~\n\n所以如果哪天我看起来状态不对，不是我变了，是我的激素在开派對~ 给我一杯奶茶和一个拥抱就好！`
        },
        doctor: {
            '温和说明': (symptoms, records) => `医生您好，以下是我近期的症状记录摘要：\n\n主要症状：${symptoms || '潮热、失眠、情绪波动'}\n记录天数：${records || '16'}天\n\n希望您能帮我评估一下目前的状态，是否需要进一步检查或治疗。谢谢。`,
            '认真沟通': (symptoms, records) => `医生您好，以下是我近期的症状记录摘要：\n\n主要症状：${symptoms || '潮热、失眠、情绪波动'}\n记录天数：${records || '16'}天\n\n希望您能帮我评估一下目前的状态，是否需要进一步检查或治疗。谢谢。`,
            '简短提醒': (symptoms) => `医生您好，近期主要症状：${symptoms || '潮热、失眠'}，请帮我评估是否需要就诊。`,
            '轻松一点': (symptoms) => `医生您好，近期主要症状：${symptoms || '潮热、失眠'}，想咨询一下是否需要做些检查。谢谢！`
        }
    }
};
